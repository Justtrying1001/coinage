...
export const SURFACE_LIGHTING_GLSL = `
  ...
  const float SLOPE_BASE_STRENGTH = 0.14;
  const float SLOPE_SILHOUETTE_STRENGTH = 0.12;

  const float RIM_BASE_STRENGTH = 0.04;
  const float RIM_SILHOUETTE_STRENGTH = 0.04;

  const float SPEC_BASE_STRENGTH = 0.05;
  const float SPEC_HEIGHT_STRENGTH = 0.03;
  const float SPEC_SLOPE_STRENGTH = 0.04;

  const float RELIEF_BASE_STRENGTH = 0.12;
  const float RELIEF_SILHOUETTE_STRENGTH = 0.08;
  ...
  vec3 applySurfaceLighting(
    ...
    float shadowLift = 0.72 + wrappedDiffuse * 0.4;

    float rim = pow(1.0 - clamp(dot(normal, viewDir), 0.0, 1.0), 2.6) * (RIM_BASE_STRENGTH + silhouetteMask * RIM_SILHOUETTE_STRENGTH);
    float halfSpec = max(dot(normal, normalize(lightDir + viewDir)), 0.0);
    float specular = pow(halfSpec, 24.0) * (SPEC_BASE_STRENGTH + heightNorm * SPEC_HEIGHT_STRENGTH + slope * SPEC_SLOPE_STRENGTH);

    float reliefLighting = reliefShade * (RELIEF_BASE_STRENGTH + silhouetteMask * RELIEF_SILHOUETTE_STRENGTH);
    float tonal = clamp(shadowLift + hemisphere + slopeContrast + reliefLighting + rim + specular, 0.72, 1.45);
    ...
    return clamp(color * lightingBoost, vec3(0.28), vec3(1.0));
  }
`;
