...
export const SURFACE_LIGHTING_GLSL = `
  ...
  const float DIFFUSE_WRAP_BASE = 0.04;
  const float DIFFUSE_WRAP_SCALE = 0.80;
  ...
  vec3 applySurfaceLighting(
    ...
    float coastWetness = coastMask * (1.0 - desertMask * 0.45);
    ...
    localRoughness = clamp(localRoughness + desertMask * 0.06 + barrenMask * 0.05 - (oceanicMask * 0.06 + shallowWater * 0.05 + deepWater * 0.04 + coastWetness * 0.03), 0.10, 0.95);
    ...
    float terrainOcclusion = (1.0 - heightNorm) * 0.12 + (1.0 - wrappedDiffuse) * 0.12;
    float shadowLift = 0.44 + wrappedDiffuse * 0.46 - terrainOcclusion - craterOcclusion * 1.18;
    float terminator = smoothstep(-0.12, 0.22, ndlRaw);
    ...
    float waterFresnelLift = pow(1.0 - clamp(dot(normal, viewDir), 0.0, 1.0), 3.4) * (shallowWater * 0.20 + deepWater * 0.14 + coastWetness * 0.08);
    ...
    float tonalBase = shadowLift + hemisphere + slopeContrast + erosionContrast + reliefLighting + rim + specular;
    float tonal = clamp(tonalBase * mix(0.78, 1.28, terminator), 0.32, 1.74);

    float luma = dot(albedo, vec3(0.2126, 0.7152, 0.0722));
    vec3 saturated = mix(vec3(luma), albedo, 1.18);
    vec3 color = saturated * tonal;
    color = mix(color, pow(max(color, vec3(0.0)), vec3(0.92)), 0.16);
    color += accentColor * (emissive * (thermalMask * 0.62 + bandMask * 0.18));

    return clamp(color * lightingBoost, vec3(0.14), vec3(1.0));
  }
`;
