export const vertexComputeFragmentShader = `
...
float evaluateNoise(vec3 point, int i){
  vec4 a = uFilterParamsA[i]; vec4 b = uFilterParamsB[i];
  float strength=a.x; float roughness=a.y; float baseRoughness=a.z; float persistence=a.w;
  float minValue=b.x; float layers=b.y; float kind=b.w;
...
void main(){
  ...
  for (int i=0;i<MAX_FILTER_COUNT;i++) {
    if (i >= uFilterLength) break;
    float v = evaluateNoise(pointOnUnitSphere, i);
    if (uFilterParamsB[i].z > 0.5) v *= mask * 7.0;
    elevation += captured ? max(0.0, v) : v;
    if (!captured) captured = true;
  }

  float scaled = max(0.0, elevation);
  gl_FragColor = vec4(pointOnUnitSphere * (1.0 + scaled), 1.0 + elevation);
}`;
